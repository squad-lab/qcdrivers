...
class DacReader:
    ...
    def _set_slot(self):
        ...
        if int(self._dac_parse(resp)) != self._slot:
            raise DACException(
                f"Unexpected return from DAC when setting slot: {resp}. DAC slot may not have been set."
            )
...
class DacChannel(InstrumentChannel, DacReader):
    ...
    def __init__(self, parent, name, channel, min_val=-5, max_val=5):
        ...
        self.add_function(
            "script_ramp",
            call_cmd=self._script_ramp,
            args=(self._volt_val, self._volt_val, self._dur_val),
        )
