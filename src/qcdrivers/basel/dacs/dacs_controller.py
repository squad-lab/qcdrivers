...
class BaselDac2Controller:
    ...
    @staticmethod
    def dacval_to_vval(dacval: int | str) -> float:
        """
        Convert a LNHR DAC II internal hexadecimal value into a voltage.

        Parameters:
        dacval: integer code or hexadecimal string used internally by the DAC

        Returns:
        float: voltage value in V
        """

        code = int(dacval.strip(), 16) if isinstance(dacval, str) else int(dacval)
        return round((code / 838860.74) - 10.000000, 6)
...
if __name__ == "__main__":
    ...
    instrument = VisaInstrument("LNHRDAC", "TCPIP0::<dac-host>::23::SOCKET")
