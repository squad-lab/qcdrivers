...
from qcdrivers.basel.dacs.dacs_controller import BaselDac2Controller
    ...
    def apply(self, awg: str) -> None:
        ...
        if nearest_frequency != desired_frequency:
            print(
                ...
                + "Changing AWG or clearing unused AWG waveforms might resolve this issue."
            )
...
class BaspiLnhrdac2Fast2d(InstrumentModule):
    ...
    def __set_2d_configuration(self, config: BaspiLnhrdac2Fast2dConfig) -> None:
        ...
        if (
            ...
            raise SystemError(
                "During the setup of the fast adaptive 2D scan, AWG A and B must not run."
            )
    ...
    def __set_2d_trigger(self, mode: str) -> None:
        ...
        if self.__current_config is None:
            raise SystemError(
                "No fast 2D scan configuration available. Set configuration parameter first."
            )
        ...
        if (
            ...
            raise SystemError(
                "During the setup of the fast adaptive 2D scan trigger, all AWGs must not run."
            )
        ...
        if mode == "disable":
            ...
            if not self.__controller.get_awg_run_state(
                ...
                raise SystemError(
                    "During the setup of the fast 2D scan point by point trigger output, AWG C and D must not run."
                )
    ...
    def __set_2d_enable(self, enable: bool) -> None:
        ...
        if enable:
            ...
            print("Fast adaptive 2D scan stopped. All AWGs can be used normally again.")
...
class BaselDac2(VisaInstrument):
    ...
    def get_help_commands(self):
        return self.__controller.get_help_commands()

    def get_help_control(self):
        return self.__controller.get_help_control()

    def get_firmware(self):
        return self.__controller.get_firmware()

    def get_serial(self):
        return self.__controller.get_serial()

    def get_health(self):
        return self.__controller.get_health()

    def get_ip(self):
        return self.__controller.get_ip()
...
if __name__ == "__main__":
    ...
    dac = BaselDac2("dac", "TCPIP0::<dac-host>::23::SOCKET")
