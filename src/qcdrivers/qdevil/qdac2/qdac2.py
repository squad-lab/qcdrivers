# The main body is copied from qcodes-contrib-drivers. Import this version with
# ``from qcdrivers.qdevil.qdac2 import QDac2``.
# Portions are Copyright (c) 2019 QCoDeS and used under the MIT License.
# Original: https://github.com/QCoDeS/Qcodes_contrib_drivers/blob/main/src/qcodes_contrib_drivers/drivers/QDevil/QDAC2.py
# License: https://github.com/QCoDeS/Qcodes_contrib_drivers/blob/main/LICENSE
...
error_ambiguous_wave = (
    "Only one of frequency_Hz or period_s can be specified for a wave form"
)
...
def diff_matrix(
    initial: Sequence[float], measurements: Sequence[Sequence[float]]
) -> np.ndarray:
...
class QDac2ExternalTrigger(InstrumentChannel):
    ...
    def __init__(self, parent: "QDac2", name: str, external: int):
        ...
        self.add_function(
            name="source_from_bus", call_cmd=f"outp:trig{external}:sour bus"
        )
        self.add_parameter(
...
class _Waveform_Context(_Channel_Context):
    ...
    def _end_marker(self, wave_kind: str) -> QDac2Trigger_Context:
        if not self._marker_end:
            self._marker_end = self.allocate_trigger()
        self._write_channel(
            f"sour{'{0}'}:{wave_kind}:mark:end {self._marker_end.value}"
        )
        return self._marker_end
    ...
    def _start_marker(self, wave_kind: str) -> QDac2Trigger_Context:
        if not self._marker_start:
            self._marker_start = self.allocate_trigger()
        self._write_channel(
            f"sour{'{0}'}:{wave_kind}:mark:star {self._marker_start.value}"
        )
        return self._marker_start
    ...
    def _period_end_marker(self, wave_kind: str) -> QDac2Trigger_Context:
        ...
        self._write_channel(
            f"sour{'{0}'}:{wave_kind}:mark:pend {self._marker_period_end.value}"
        )
    ...
    def _period_start_marker(self, wave_kind: str) -> QDac2Trigger_Context:
        ...
        self._write_channel(
            f"sour{'{0}'}:{wave_kind}:mark:pstart {self._marker_period_start.value}"
        )
...
class Square_Context(_Waveform_Context):
    ...
    def _set_frequency(
        self, frequency_Hz: Optional[float], period_s: Optional[float]
    ) -> None:
        if frequency_Hz:
...
class QDac2Channel(InstrumentChannel):
    ...
    def measurement(
        ...
        if aperture_s and nplc:
            raise ValueError(
                "Only one of nplc or aperture_s can be "
                "specified for a current measurement"
            )
        if not aperture_s and not nplc:
            nplc = 1
        return Measurement_Context(
            self, delay_s, repetitions, current_range, aperture_s, nplc
        )
    ...
    def dc_list(
        ...
        """
        return List_Context(
            self, voltages, repetitions, dwell_s, delay_s, backwards, stepped
        )
...
class Trace_Context:
    ...
    def waveform(self, values: Sequence[float]) -> None:
        ...
        if len(values) != self.size:
            raise ValueError(
                f"trace length {len(values)} does not match "
...
class Virtual_Sweep_Context:
    ...
    def _route_inner_trigger(self) -> None:
        ...
        channel.write_channel(
            f"sour{'{0}'}:dc:mark:sst {_trigger_context_to_value(trigger)}"
        )
    ...
    def _send_list_to_qdac(self, contact_index, voltages):
        ...
        dc_list = channel.dc_list(
            voltages=voltages, dwell_s=self._step_time_s, repetitions=self._repetitions
        )
        trigger = self._arrangement.get_trigger_by_name(self._start_trigger_name)
...
class Arrangement_Context:
    ...
    def _allocate_internal_triggers(
        self, internal_triggers: Optional[Sequence[str]]
    ) -> None:
    ...
    def virtual_sweep(
        ...
        return Virtual_Sweep_Context(
            self, sweep, start_sweep_trigger, step_time_s, step_trigger, repetitions
        )

    def _calculate_1d_values(
        self, contact: str, voltages: Sequence[float]
    ) -> np.ndarray:
    ...
    def virtual_sweep2d(
        ...
        sweep = self._calculate_2d_values(
            inner_contact, inner_voltages, outer_contact, outer_voltages
        )
        ctx = Virtual_Sweep_Context(
            self,
            sweep,
            start_sweep_trigger,
            inner_step_time_s,
    ...
    def _setup_outer_trigger(
        self,
        outer_step_trigger: str,
        ...
        if not self._outer_trigger_channel:
            raise ValueError(
                "Arrangement needs an outer_trigger_channel when"
    ...
    def virtual_detune(
        ...
        return Virtual_Sweep_Context(
            self, sweep, start_trigger, step_time_s, step_trigger, repetitions
        )
    ...
    def _check_same_lengths(contacts, start_V, end_V) -> None:
        ...
        if n_contacts != len(start_V):
            raise ValueError(
                f"There must be exactly one voltage per contact: {start_V}"
            )
    ...
    def _calculate_detune_values(
        ...
        sweep = list()
        forward_V = [
            forward_and_back(start_V[i], end_V[i], steps) for i in range(len(contacts))
        ]
    ...
    def leakage(self, modulation_V: float, nplc: int = 2) -> np.ndarray:
        ...
        """
        steady_state_A, currents_matrix = self._leakage_currents(
            modulation_V, nplc, "low"
        )
    ...
    def _allocate_external_triggers(
        self, output_triggers: Optional[Dict[str, int]]
    ) -> None:
...
class QDac2(VisaInstrument):
    ...
    def connect_external_trigger(
        self, port: int, trigger: QDac2Trigger_Context, width_s: float = 1e-6
    ) -> None:
    ...
    def mac(self) -> str:
        ...
        return f"{mac[1:3]}-{mac[3:5]}-{mac[5:7]}-{mac[7:9]}-{mac[9:11]}-{mac[11:13]}"
    ...
    def arrange(
        ...
        """
        return Arrangement_Context(
            self, contacts, output_triggers, internal_triggers, outer_trigger_channel
        )
    ...
    def _check_for_wrong_model(self) -> None:
        ...
        if model != "QDAC-II":
            raise ValueError(
                f"Unknown model {model}. Are you using the right"
    ...
    def _check_for_incompatiable_firmware(self) -> None:
        ...
        if parse(firmware) < parse(least_compatible_fw):
            raise ValueError(
                f"Incompatible firmware {firmware}. You need at "
    ...
    def _set_up_external_triggers(self) -> None:
        triggers = ChannelList(
            self, "Channels", QDac2ExternalTrigger, snapshotable=False
        )
