/*
  based on Vrekrer_scpi_parser library.
  the header file of the library needs to be modified to allow very long scpi strings
*/
...
float DAC_voltage = 0; // global variable to keep track of voltage set to the DAC at DAC0, initially DAC0 is not set
...
void Identify(SCPI_C commands, SCPI_P parameters, Stream& interface) {
  interface.println(F("AG-BLUHM , Arduino Multiplexer v1.0 , 123, V 1.0")); // important to identify mulitple muxes by serial
}

// open all switches by shutting down all ICs
void Reset(SCPI_C commands, SCPI_P parameters, Stream& interface) {
  for (int i = 1; i <= 12; i++) ShutDownChip(i); //BUG: doesnt work sometimes
...
void GetStatus(SCPI_C commands, SCPI_P parameters, Stream& interface) {
  if (parameters.Size() == 2) // param1 is PIN, param2 is OUT
  {
    ...
    //if (PIN>=49) PIN=PIN+2; // why did I put it??
    interface.println(PINtoMStatus(PIN, OUT)); // connected:1, disconnected: 0, invalid input: 128, communication error: 255
  }
  else
  {
...
void SetStatus(SCPI_C commands, SCPI_P parameters, Stream& interface) {
  if (parameters.Size() == 2) // param1 is PIN, param2 is OUT
  {
    ...
    if (last_header.compareTo("ON") == 0)  interface.println(PINtoM(PIN, OUT, 1)); // successful:0 , not successful otherwise
    else if (last_header.compareTo("OFF") == 0)  interface.println(PINtoM(PIN, OUT, 0));
    else  interface.println("Usage: MUX:SWITCH:ON/OFF PIN, OUT");
