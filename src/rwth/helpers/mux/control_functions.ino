/**
   This sketch contains miscellaneous functions used to switch and check connections on the board
...
/**
   General function that dis-/connects PIN to COMMON OUTPUT M(= 1,2,3,4)
*/
...
uint8_t PINtoM(uint8_t PIN, uint8_t M, uint8_t state) {
  ...
  if (  ((PIN >= 1 && PIN <= 48) || (PIN >= 51 && PIN <= 98) ) && (M >= 1 && M <= 4) && (state == 0 || state == 1)) {
    ...
    if (state == 1) { // switch on
      ...
      val += Wire.endTransmission();
      // how big is the overhead of this (below) and is it worth it i.e. does it lead to noise/interference reduction or not?
      bool IC_Idle = isIdle(ICNum); // hidden caveat , val is not incremented by i2c communication within isIdle, i think it is safe cuz we already talked with the IC
...
/**
   Tells the status of PIN to M connection
   0  : off
   1  : on
   128: invalid PIN/M
   255: failed I2C communication somewhere
*/
...
uint8_t PINtoMStatus(uint8_t PIN, uint8_t M) {
  ...
  if (  ((PIN >= 1 && PIN <= 48) || (PIN >= 51 && PIN <= 98) ) && (M >= 1 && M <= 4)) { // valid inputs
    ...
    else { // sucessfull I2C communication
