...
class CryogenicSMS120C(VisaInstrument):
    ...
    def __init__(
        ...
        name,
        address,
        coil_constant=0.113375,
        current_rating=105.84,
        ...
        **kwargs,
        ...
        if "terminator" in kwargs.keys():
            ...
            log.warning(
                "Passing terminator to CryogenicSMS is no longer supported and has no effect"
            )
        ...
        self._field_rating = (
            coil_constant * current_rating
        )  # corresponding max field based
        ...
        self.add_parameter(
            ...
            vals=Numbers(
                -self._field_rating,  # i.e. ~12T, calculated
                self._field_rating,
            ),
        )
    ...
    def _get_persistentMode(self):
        ...
        if value is not None and ("OFF" in value) and (abs(field) >= 0.005):
            ...
            if units == 1:
                log.info(
                    "Magnet in persistent mode, at a field of %f T" % persistentField
                )
            elif units == 0:
                log.info(
                    "Magnet in persistent mode, at a field of %f A" % persistentField
                )
            persistentMode = True
        else:
    ...
    def _get_unit(self):
        ...
        raise RuntimeError(
            f"Could not reliably determine unit from TESLA response. Last reply: {last!r}"
        )
    ...
    def _get_maxField(
        self,
    ):  # Get the maximum B field, returns a float (in Amps or Tesla)
    ...
    def _get_field(self):
        ...
        for _ in range(3):
            ...
            m = re.match(
                r"({}) TESLA AT ({}) VOLTS".format(
                    CryogenicSMS120C._re_float_exp, CryogenicSMS120C._re_float_exp
                ),
                value,
            )
            if m:
        ...
        raise RuntimeError(
            f"GET OUTPUT returned unexpected reply (cannot parse TESLA field). Last reply: {last!r}"
        )
    ...
    def _set_switchHeater(self, val):  # Turn heater ON(1) or OFF(0)
        if self._get_rampStatus() == 1:
            log.error("Cannot switch heater during a ramp, first pause the controller.")
        else:
    ...
    def _set_persistentMode(self, val):
        if self._get_rampStatus() == 0:  # Check magnet on HOLD
            ...
            if val == 1:
                if self._get_persistentMode() == False and (
                    self._get_switchHeater() == True
                ):
                    ...
                    while True:
                        if self._get_rampStatus() == 0:
                            ...
                            persistentField = self._get_persistentField()
                            log.info(
                                "Magnet is in persistent mode at Field = %f."
                                % persistentField
                            )
                            break
            ...
            # Exit persistent mode
                ...
                persistentField = self._get_persistentField()
                if self._get_persistentMode() == True and (
                    self._get_switchHeater() == False
                ):
                    log.info(
                        "Exiting persistent mode from a field of %f" % persistentField
                    )
                    switchHeater = 1
                    strHeaterStatus = self._ask_retry("HEATER %d" % switchHeater)
                    log.info(strHeaterStatus)
                    ...
                    time.sleep(30)
                    log.info(
                        "Matching magnet lead current to persistent field of %f..."
                        % persistentField
                    )
    ...
    def _set_rampRate(self, val):
        ...
        if val <= self._current_ramp_limit:
            ...
            self._ask_retry(
                "SET RAMP %0.2f" % CryogenicSMS120C.default_current_ramp_limit
            )
    ...
    def _can_startRamping(self):
        ...
        if self._get_rampRate() <= self._current_ramp_limit:
            if state == 2:  # Quench
                ...
                return True
            log.error("Could not ramp, magnet in state: {}".format(state))
            return False
        else:
            log.warning(
    ...
    def _set_field(self, val):
        ...
        if val < 0:
            raise ValueError(
                "_set_field expects a non-negative magnitude; use _set_field_bidirectional for signed fields."
            )

        if not self.switchHeater():  # If switch heater is OFF
            log.error(
                "Unable to set field, switch heater is off, persistent mode may be active"
            )
            return
        ...
        if self._can_startRamping():
            ...
            if (self._get_unit() == 1 and (val <= self._get_maxField())) or (
                ...
                self._ask_retry(
                    "SET MID %0.3f" % val
                )  # Set target field, CHANGED FROM 0.2 to 0.3
                ...
                self._wait_for_field(
                    signed_target
                )  # , refresh_time=0.05)   # ADDED WAIT UNTIL FIELD IS ACTUALLY REACHED
    ...
    def _set_field_bidirectional_fast(self, val: float) -> None:
        ...
        if st in (2, 3, 4):
            raise RuntimeError(
                f"Magnet in error state {st} (2=QUENCH,3=EXTERNAL,4=FAULT)."
            )
